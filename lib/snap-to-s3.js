...
class SnapToS3 {
	 ...
	 * @returns {Promise}
	 */
	_uploadProcessStdOut(sourceProcess, streamLengthEstimate, tags, _s3Params, logger) {
		let
			failed = false,
			uploader = null,
			...
			uploadPromise = new Promise((resolve, reject) => {
				...
				uploader = new AWS.S3.ManagedUpload({
					...
					partSize: Math.max(
						...
						Math.ceil((streamLengthEstimate + 10 * 1024 * 1024) / (AWS.S3.ManagedUpload.maxTotalParts * 0.9)),
						AWS.S3.ManagedUpload.minPartSize
					),
	 ...
	 * @returns {Promise.<EC2.Volume>}
	 */
	_createTemporaryVolumeFromSnapshot(snapshotID) {
		...
		return this._findTemporaryVolumeForSnapshot(snapshot)
			.then(existingVolume => {
				if (existingVolume) {
					...
					return this._createTemporaryVolumeFromSnapshot(snapshot.SnapshotId)
	...
	_uploadPartitionsUsingDd(partitions, snapshot, tags, s3Params, logger) {
		...
		let
			...
			promise = this._uploadProcessStdOut(dd, drive.SIZE, tags, s3Params, logger);
		...
		if (this.options.validate) {
			promise = promise
				...
				.then(ddObjectHead => {
					...
					return this._validateFileAgainstCompressedS3File(drive.DEVICEPATH, drive.SIZE, s3Params.Key, ddObjectHead.ContentLength);
				})
			...
			promise = promise.then(() => {
				...
				promise = this._mountTemporaryVolume(partition, mountPoint, logger)
					...
					.then((_mountSize) => {
						...
						return this._uploadProcessStdOut(tar, mountSize, tags, s3Params, logger)
					});
					...
					promise = promise
						.then(() => {
							...
							return this.s3.headObject({
								...
								.then(
									s3Head => this._validateDirectoryAgainstS3Tar(mountPoint, mountSize, s3Params.Key, s3Head.ContentLength),
									error => {
	 ...
	 */
	_validateFileAgainstCompressedS3File(filename, fileSize, s3Key, s3Size) {
		let
	 ...
	 */
	_validateDirectoryAgainstS3Tar(directory, directorySize, s3Key, s3Size) {
	...
	_validatePartitionsUsingDd(partitions, snapshot, s3Key, s3Size, logger) {
		let
			drive = filterBlockDevicesToGetRawDisk(partitions);
		...
		return this._validateFileAgainstCompressedS3File(drive.DEVICEPATH, drive.SIZE, s3Key, s3Size)
			.then(hash => this._reportVolumeDdHashSuccess(snapshot, hash, logger));
	}
					...
					return this.s3.headObject({
						...
						s3Head => this._mountTemporaryVolume(partition, mountPoint, logger)
							...
							.then(mountSize => {
								...
								return this._validateDirectoryAgainstS3Tar(mountPoint, mountSize, s3Key, s3Head.ContentLength);
							})
		...
		return this.s3.headObject({
			...
			.then(ddObjectHead => {
				...
				return awsTools.waitForVolumePartitions(volume, this.instanceIdentity.instanceId, PARTITION_POLL_MAX_RETRY, PARTITION_POLL_INTERVAL)
					.then(partitions => {
						if (ddObjectHead) {
							return this._validatePartitionsUsingDd(partitions, snapshot, ddKey, ddObjectHead.ContentLength, logger);
						} else {
